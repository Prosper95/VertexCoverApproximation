...
public class VertexCoverVerification {
	...
	public static void main(String[] args) {
		...
		try {
			candSol = new Scanner(new File(args[1]));
		} catch (FileNotFoundException e) {
			System.out.println("File did not open");
			System.exit(1);
		}
		...
		while(candSol.hasNext()) {
			solution.add(candSol.next());
		}
		...
		for( int i = 0; i < solution.size(); i++) {
			String v = solution.get(i);
			
			ArrayList<Vertex> toRemove = new ArrayList<Vertex>();
			Iterator<Vertex> iter = g.adjacentTo(v).iterator();
			
			while (iter.hasNext()) {
				toRemove.add(iter.next());
			}
			
			for(Vertex t : toRemove) {
				g.removeEdge(v, t.getName());
			}
		}
